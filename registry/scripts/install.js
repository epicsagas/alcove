...
async function install() {
  ...
  if (platform === "darwin") {
    ...
    if (hasBrew) {
      ...
      const r = spawnSync("brew", ["install", "epicsagas/tap/alcove"], {
        stdio: "inherit",
        shell: false,
      });
  ...
  if (platform === "win32") {
    log("alcove is not available for Windows via installer script.");
    log("Install via: cargo install alcove");
    process.exit(0);
  }
