...
```bash
# 포그라운드에서 서버 시작
alcove serve                       # 기본값: 127.0.0.1:57384
...
```
백그라운드 서버 있음 (프록시 모드):
  에이전트 ──stdio──→ alcove (경량 프록시, <10 MB, 즉시 시작)
                       │ stdin → HTTP POST /mcp
                       │ HTTP 응답 → stdout
                       └──HTTP──→ alcove serve (상시 대기)
                                   ├─ BM25 인덱스 (로드됨)
                                   ├─ ONNX 임베딩 모델 (로드됨)
                                   ├─ HNSW 벡터 인덱스 (로드됨)
                                   └─ 하이브리드 검색 즉시 가능 (~5ms)
...
백그라운드 서버 없음 (직접 모드):
  에이전트 ──stdio──→ alcove (전체 엔진, 세션마다 프로세스 생성)
                       ├─ ONNX 임베딩 모델 로드 (2-5초 콜드 스타트)
                       ├─ BM25 인덱스 열기
                       ├─ HNSW 벡터 인덱스 구축
                       └─ 하이브리드 검색 가능 (워밍업 후)
```

시작 시 stdio 프로세스가 설정된 호스트/포트로 `GET /health`를 확인합니다. 서버가 응답하면 자동으로 프록시 모드로 진입합니다 — **MCP 설정 변경 불필요**. JSON-RPC 페이로드는 양쪽 모드에서 동일하며, 내부 전송 방식만 다릅니다.

> **세션마다 프로세스 생성:** 직접 모드에서는 에이전트 세션마다 별도의 `alcove` 프로세스(~10 MB)가 생성됩니다. 여러 에이전트를 동시에 사용하면 각자 인덱스를 로드합니다. 백그라운드 서버를 활성화하면 모든 세션이 단일 상시 프로세스를 공유합니다.

#### stdio에서 상시 HTTP로 전환하기

setup 당시 백그라운드 서버를 건너뛰었거나, 이전 버전에서 설치한 경우 MCP 설정을 변경하지 않고도 언제든 활성화할 수 있습니다:

```bash
# 1. 백그라운드 서버 활성화 (macOS 로그인 항목으로 등록, 즉시 시작)
alcove enable

# 2. 실행 확인
alcove start   # 이미 실행 중이면 no-op, PID 출력

# 3. 셸에 ALCOVE_TOKEN 설정 (setup 시 자동 추가되지만 확인)
echo $ALCOVE_TOKEN          # 토큰이 출력되어야 함
# 비어있으면 셸 프로파일에 추가:
alcove token                # 저장된 토큰 출력
echo 'export ALCOVE_TOKEN="'$(alcove token)'"' >> ~/.zshrc
source ~/.zshrc
```

기존 stdio MCP 설정(`command: alcove`)은 그대로 유지합니다. 서버가 실행되면 다음 에이전트 세션부터 자동으로 프록시 모드로 전환됩니다 — 에이전트 재시작 없이 새 세션만 열면 됩니다.
