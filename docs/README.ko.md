...
Alcove는 AI 코딩 에이전트가 필요할 때 프라이빗 프로젝트 문서에 접근할 수 있게 해주는 HTTP API 서버입니다 — **BM25 + 벡터 하이브리드 검색**으로 정밀 검색, **tree-sitter 코드 인덱싱**으로 에이전트가 코드베이스 구조를 이해, **정책 강제**로 문서 일관성 유지. 컨텍스트 팽창 없이, 공개 저장소 유출 없이, 에이전트별 프로젝트 설정 없이.

PRD, 아키텍처 결정, 시크릿 맵, 내부 런북을 한 곳에 보관하세요. 모든 AI 에이전트가 같은 도구를 얻고, 모든 프로젝트에서 동작하며, 프로젝트별 설정이 필요 없습니다.

## 데모

![Alcove 에이전트 데모](../demo-agent.gif)
> *Claude, Codex — 검색 · 프로젝트 전환 · 글로벌 검색 · 검증 & 생성. 한 번의 설정.*

<details>
<summary>CLI 데모</summary>

![Alcove CLI 데모](../demo.gif)
...
Alcove는 모든 프라이빗 문서를 프로젝트별로 정리된 **하나의 공유 저장소**에 보관합니다. HTTP API를 통해 모든 에이전트가 동일한 방식으로 접근할 수 있습니다 — Claude Code, Cursor, Codex 어디서든 상관없습니다.
...
- **한 번 설정, 모든 에이전트** — 한 번 설정하면 모든 AI 에이전트가 동일한 접근 권한을 얻음
...
> **필수**: 설치 후 `alcove setup`을 한 번 실행하여 문서 루트를 설정하고 전체 기능을 활성화하세요. 플러그인은 API 서버를 자동으로 시작하지만, `setup`이 실행되지 않으면 Alcove가 문서를 검색하거나 인덱싱할 수 없습니다.
>
...
바이너리를 자동 설치하고 다음 세션 시작 시 API 서버를 시작합니다.
...
플러그인(API 서버, 스킬, 훅)을 자동으로 설치하고 다음 세션 시작 시 시작합니다.
...
다음 세션 시작 시 바이너리를 자동 설치하고 API 서버를 구동합니다.
...
```bash
cargo binstall alcove   # 사전 빌드 바이너리, 하이브리드 검색 포함
cargo install alcove --features full-macos   # 소스 빌드 (macOS)
cargo install alcove --features full-cross   # 소스 빌드 (Linux/Windows)
```

> **참고**: `cargo binstall`은 하이브리드 검색(벡터 + BM25)이 포함된 사전 빌드 바이너리를 다운로드합니다. 소스에서 빌드할 때는 하이브리드 검색을 위해 `--features full-macos` 또는 `--features full-cross`가 필요합니다. features 없이 빌드하면 BM25(키워드) 검색만 사용할 수 있습니다.
...
6. 어떤 AI 에이전트를 설정할지 (스킬 파일 — Claude Code와 Codex는 플러그인 시스템으로 처리됩니다)
...
**에이전트가 Alcove 도구를 찾지 못함**
`alcove setup`을 다시 실행하세요 — 모든 설정된 에이전트의 API 서버를 재설정합니다. 그 후 새 에이전트 세션을 시작하세요 (변경 사항은 다음 세션 시작 시 적용됩니다).

**검색 결과가 없음**
인덱스가 아직 빌드되지 않았을 수 있습니다. `alcove index`를 실행하여 빌드한 후 다시 시도하세요.

**백그라운드 서버에서 403 Unauthorized**
셸에 `ALCOVE_TOKEN`이 설정되지 않았습니다. `alcove token`으로 토큰을 출력한 후, 셸 프로필에 `export ALCOVE_TOKEN="..."`을 추가하고 다시 로드하세요.

**`alcove doctor`가 문제를 보고함**
`doctor`가 출력하는 제안을 따르세요 — 바이너리 위치, API 서버 상태, 인덱스 상태, `pdftotext` 등 선택적 의존성을 확인합니다.
...
AI 코딩 에이전트는 **로컬 HTTP API**(포트 58301)를 통해 Alcove를 사용합니다. 스킬이 내부적으로 `curl http://localhost:58301/...`을 호출합니다. 사용자가 직접 이 API를 호출할 필요는 없으며, 에이전트가 프로젝트에 대해 질문을 받으면 자동으로 호출합니다.
...
flowchart LR
    ...
    subgraph Agents["모든 AI 에이전트"]
        AG["Claude Code · Cursor\nCodex · Copilot\n+4 more"]
    end

    subgraph API["Alcove HTTP API 서버"]
        T["search · get_file\noverview · audit\ninit · validate"]
    end
...
문서는 별도 디렉토리(`DOCS_ROOT`)에 프로젝트별 폴더로 정리됩니다. Alcove는 HTTP 58301 포트를 통해 AI 에이전트에게 문서를 제공합니다.
...
alcove mcp <CMD>      백그라운드 API 서버 관리 (start, stop, status, enable, disable)
```
...
> **참고**: MCP는 stdio 기반 접근을 선호하는 사용자를 위해 여전히 사용 가능합니다. 수동 MCP 설정은 `registry/mcp.json`을 참조하세요.
...
| 모델 | 디스크 | 차원 | 컨텍스트 | 언어 | 추천 용도 | 피크 RAM |
|------|--------|------|----------|------|-----------|----------|
| **`ArcticEmbedXS`** (기본) | **90 MB** | **384** | **512** | **다국어** | **기본 — 최고 가성비** | **~400 MB** |
| `ArcticEmbedXSQ` | 90 MB | 384 | 512 | 다국어 | 양자화, 더 작은 다운로드 | ~400 MB |
| `MultilingualE5Small` | 470 MB | 384 | 512 | 100+ 언어 | 한국어/CJK 최고 품질 | ~1.2 GB |
| `BGEM3` | 600 MB | 1024 | 8192 | 100+ 언어 | 프리미엄 — Dense+Sparse+ColBERT | ~2 GB |
| `ArcticEmbedMLong` | 430 MB | 768 | 8192 | 다국어 | 장문서 | ~1.5 GB |
| `JinaEmbeddingsV2BaseCode` | 550 MB | 768 | 8192 | 코드+영어 | 코드 문서 | ~1.5 GB |

기본 모델은 **ArcticEmbedXS** (90 MB, 다국어)입니다. 대부분의 프로젝트에서 크기와 품질의 최적 균형을 제공합니다.

임베딩 모델은 [fastembed-rs](https://github.com/Anush008/fastembed-rs) (ONNX Runtime) 기반이며 완전히 로컬에서 실행됩니다. 다른 모델을 사용하려면 `config.toml`에서 설정하세요:

```toml
[embedding]
model = "BGEM3"    # 모델 문서의 Variable 이름
```

40개 이상의 지원 모델 전체(차원, 컨텍스트 길이, 언어 지원)는 **[EMBEDDING_MODELS.md](../docs/EMBEDDING_MODELS.md)**에서 확인할 수 있습니다.
...
인덱스는 API 서버 시작 시 백그라운드에서 자동으로 빌드되며, 파일 변경을 감지하면 자동으로 재빌드합니다. 크론 잡도, 수동 작업도 필요 없습니다.
...
alcove model set ArcticEmbedXS
alcove model download   # ~90 MB 다운로드
...
**rebuild 시 메모리:**
피크 RAM은 모델에 따라 다릅니다 — 위 테이블의 "피크 RAM" 컬럼을 참조하세요. 대형 모델(BGEM3, ArcticEmbedMLong)은 rebuild 중 1.5-2 GB를 사용할 수 있습니다. rebuild 완료 후 평상시 메모리는 `[memory]` 설정에 따라 50-200 MB 수준으로 감소합니다. `max_hnsw_cache`를 낮추고 `model_unload_secs`를 짧게 설정하면 평상시 메모리를 더 줄일 수 있습니다.
...
단일 바이너리, 런타임 의존성 없음. Tantivy는 최고 수준의 BM25 구현입니다. fastembed (ONNX Runtime) 덕분에 Python 없이 로컬 벡터 임베딩을 사용할 수 있습니다. `cargo install` 하나나 curl 한 번이면 됩니다 — Docker도, Node.js도, virtualenv도 필요 없습니다.
...
- **다중 사용자 원격 접근** — LAN/VPN을 통한 팀 문서 공유 (베어러 토큰 인증, 속도 제한 이미 구현됨). 필요: 쓰기 API, 동시 인덱스 조정, 프로젝트 생명주기 관리.
