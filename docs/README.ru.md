...
alcove token        Вывести токен доступа для совместной работы
...
alcove api <CMD>      Управление жизненным циклом фонового REST API-сервера (start, stop, status, enable, disable)
...
Запуск постоянного фонового сервера устраняет задержку при холодном старте (загрузка модели ONNX занимает от 2 до 5 секунд) при каждой новой сессии агента. **`alcove setup` включает это по умолчанию** (объект входа в систему macOS).

```bash
# Включить и запустить (сохраняется после перезагрузки — macOS)
alcove mcp enable --now

# Жизненный цикл
alcove mcp stop / start / restart / status

# Отключить и удалить объект входа в систему
alcove mcp disable
```

Вы также можете запустить отдельный сервер REST API:

```bash
# Запустить API-сервер в фоновом режиме
alcove api start
```

Сервер использует bearer-токен для аутентификации — он создается автоматически во время `alcove setup` и сохраняется в `config.toml`. Ваша существующая конфигурация MCP (`command: alcove`) остается неизменной; процесс stdio автоматически обнаруживает запущенный сервер и работает как прокси-сервер для него.

```bash
# Проверить или поделиться токеном
alcove token

# Установить в профиле оболочки (установка делает это автоматически)
export ALCOVE_TOKEN="alcove-..."
```

Приоритет токена: флаг `--token` > переменная окружения `ALCOVE_TOKEN` > `config.toml`.

Логи записываются в `~/.alcove/logs/`. При запуске выполните `alcove doctor`, чтобы убедиться, что сервер доступен.
