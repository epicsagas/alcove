...
5. **HTTP-сервер** — хост, порт, автоматически сгенерированный bearer-токен, и регистрация элемента входа в систему
...
| `index_code_structure` | Анализирует исходный код с помощью tree-sitter и генерирует `CODE_INDEX.md` для каждого проекта (требуется функция `code-index`) |
...
alcove index-code   Генерирует индекс структуры кода из исходников (требуется функция code-index)
