...
Alcove आपके सभी प्राइवेट दस्तावेज़ों को **एक साझा रिपॉज़िटरी** में रखता है, प्रोजेक्ट के अनुसार व्यवस्थित। कोई भी MCP-संगत एजेंट उन्हें एक ही तरीके से एक्सेस करता है — चाहे आप Claude Code में हों, Cursor में, या Codex में।
...
| मॉडल | डिस्क | आयाम | भाषाएँ | सर्वोत्तम उपयोग | पीक RAM |
|-------|-------|------|--------|----------------|---------|
| `AllMiniLML6V2` | 90 MB | 384 | अंग्रेज़ी | न्यूनतम फुटप्रिंट, तेज़ अंग्रेज़ी-मात्र इंडेक्सिंग | ~400 MB |
| **`MultilingualE5Small`** | **235 MB** | **384** | **100+ भाषाएँ** | **डिफ़ॉल्ट — बहुभाषी / मिश्रित-भाषा प्रोजेक्ट** | **~700 MB** |
| `MultilingualE5Base` | 555 MB | 768 | 100+ भाषाएँ | बेहतर बहुभाषी गुणवत्ता | ~2 GB |
| `MultilingualE5Large` | 2.2 GB | 1024 | 100+ भाषाएँ | अधिकतम बहुभाषी गुणवत्ता | ~7 GB |
| `BGEM3` | 2.3 GB | 1024 | 100+ भाषाएँ | अत्याधुनिक बहुभाषी | ~8 GB |
| `ArcticEmbedXS` | 90 MB | 384 | अंग्रेज़ी | Snowflake — 384 dim में सर्वोत्तम गुणवत्ता | ~400 MB |
| `ArcticEmbedS` | 130 MB | 384 | अंग्रेज़ी | Snowflake — छोटे आकार में बेहतर पुनर्प्राप्ति | ~500 MB |
| `ArcticEmbedM` | 430 MB | 768 | अंग्रेज़ी | Snowflake — व्यावहारिक पुनर्प्राप्ति गुणवत्ता | ~1.5 GB |
| `ArcticEmbedL` | 1.3 GB | 1024 | अंग्रेज़ी | Snowflake — बंद-स्रोत API के प्रतिस्पर्धी गुणवत्ता | ~5 GB |
...
पीक RAM मॉडल के अनुसार भिन्न होती है — ऊपर दी गई तालिका में "पीक RAM" कॉलम देखें। बड़े मॉडल (BGEM3, MultilingualE5Large, ArcticEmbedL) rebuild के दौरान 5-10 GB उपयोग कर सकते हैं। rebuild पूरा होने के बाद, स्थिर अवस्था आपकी `[memory]` कॉन्फ़िगरेशन के अनुसार ~50-200 MB तक गिर जाती है। कम `max_hnsw_cache` और छोटे `model_unload_secs` से स्थिर मेमोरी को और कम किया जा सकता है।
...
एकल बाइनरी, कोई रनटाइम निर्भरता नहीं। Tantivy सबसे उत्तम BM25 है। candle-transformers हमें ONNX या Python के बिना लोकल वेक्टर एम्बेडिंग देता है। एक `cargo install` या curl — कोई Docker नहीं, कोई Node.js नहीं, कोई virtualenv नहीं।
