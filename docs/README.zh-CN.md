...
> **必选**: 安装后运行一次 `alcove setup` 来配置文档根目录并启用全部功能。插件会自动注册 MCP 连接，但在运行 `setup` 之前，Alcove 无法搜索或索引文档。
>
...
自动安装插件（MCP 服务器、技能、钩子）并在下次会话启动时注册。
...
**代理找不到 Alcove 工具**
重新运行 `alcove setup` — 它会重新注册所有已配置代理的 MCP 服务器。然后启动新的代理会话（注册在下次会话启动时生效）。
...
**`alcove doctor` 报告问题**
按照 `doctor` 输出的建议操作 — 它会检查二进制位置、MCP 注册、索引状态和 `pdftotext` 等可选依赖。
...
单一二进制文件，无运行时依赖。Tantivy 是业界领先的 BM25 引擎。candle-transformers 让我们无需 ONNX 或 Python 即可实现本地向量嵌入。一条 `cargo install` 或 curl 命令即可完成安装——无需 Docker、无需 Node.js、无需虚拟环境。
