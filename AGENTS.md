...
- **Destructive**: `alcove rebuild` (and `alcove vault rebuild`) wipe and rebuild from scratch — requires user approval and `--yes` in non-interactive sessions
...
- NEVER run `alcove rebuild` / `alcove vault rebuild` (destructive full rebuild) without explicit user approval; non-interactive runs require `--yes`
