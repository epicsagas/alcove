...
fn handle_tools_list(id: Option<Value>) -> RpcResponse {
    let tools: Vec<ToolDescription> = vec![
        ...
        ToolDescription {
            ...
            description: concat!(
                "Build or rebuild the full-text BM25 search index for all projects in the alcove doc-repo.\n",
                "\n",
                "Call this tool after adding, updating, or deleting documentation files so that ",
                "search_project_docs returns accurate ranked results. Also call it when the user explicitly ",
                "asks to reindex, or when check_doc_changes reports pending changes.\n",
                "\n",
                "Without a built index, search_project_docs falls back to grep (substring match). ",
                "After rebuilding, it uses BM25 ranked search for more relevant results.\n",
                "\n",
                "This operation rewrites the index on disk. It may take a few seconds for large doc repos. ",
                "Returns a summary of indexed files and projects on success."
            ).into(),
            input_schema: json!({
                "type": "object",
                "properties": {},
