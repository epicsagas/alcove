...
pub fn tool_get_ir_slice(project_root: &Path, args_value: Value) -> Result<Value> {
    let args: GetIrSliceArgs = serde_json::from_value(args_value)
        .context("get_ir_slice requires { path }")?;
    ...
    // Resolve the markdown file — also check sidecar path like tool_get_doc_ir
    let sidecar_path = md_path.with_extension("ir.json");
    let sidecar_fresh = sidecar_path.exists() && {
        match (md_path.metadata(), sidecar_path.metadata()) {
            (Ok(md_meta), Ok(ir_meta)) => md_meta.modified().ok() <= ir_meta.modified().ok(),
    ...
    let ir_doc: ai_ir::IrDocument = if sidecar_fresh {
        let content = std::fs::read_to_string(&sidecar_path)?;
        ai_ir::IrDocument::from_json(&content).context("Failed to deserialise IR sidecar")?
    } else {
        let md_fallback = PathBuf::from(
            sidecar_path
                .to_str()
        ...
        const MAX_MD_BYTES: u64 = 10 * 1024 * 1024;
        let md_size = md_fallback.metadata().map(|m| m.len()).unwrap_or(0);
        if md_size > MAX_MD_BYTES {
            anyhow::bail!(
                "Markdown file too large for on-the-fly IR generation ({} bytes, limit {} bytes)",
        ...
        let md_content = std::fs::read_to_string(&md_fallback)
            .context("Failed to read markdown file for on-the-fly IR generation")?;
        ai_ir::Transpiler::from_markdown(md_fallback.to_str().unwrap_or(""), &md_content)
    };

    let use_json = args.format == "json";

    // Build the slice
    let slice_opt: Option<ai_ir::IrSlice> = if let Some(ref q) = args.query {
        ir_doc.slice_by_entity(q)
    } else {
        // No query — return all constraints as a slice
        Some(ai_ir::IrSlice {
            entities: ir_doc.entities.clone(),
            constraints: ir_doc.constraints.clone(),
        })
    };

    match slice_opt {
        None => Ok(json!({"slice": "", "matched": 0})),
        Some(slice) => {
            let matched = slice.constraints.len();
            if use_json {
                Ok(json!({
                    "slice": serde_json::to_value(&slice)?,
                    "matched": matched,
                }))
            } else {
                Ok(json!({
...
mod tests {
    ...
    #[test]
    fn test_get_ir_slice_compact_no_query() {
        let tmp = TempDir::new().unwrap();
        let md = "# Auth\n\nUsers **must** authenticate before accessing any endpoint.\n";
        let md_path = tmp.path().join("CONVENTIONS.md");
        std::fs::write(&md_path, md).unwrap();

        let result = tool_get_ir_slice(tmp.path(), json!({"path": "CONVENTIONS.md"})).unwrap();
        assert_eq!(result["matched"].as_u64().unwrap(), result["matched"].as_u64().unwrap()); // exists
        assert!(result["slice"].is_string());
    }

    #[test]
    fn test_get_ir_slice_query_matched() {
        let tmp = TempDir::new().unwrap();
        let md = "# Auth\n\nUsers **must** authenticate before accessing any endpoint.\n";
        let md_path = tmp.path().join("CONVENTIONS.md");
        std::fs::write(&md_path, md).unwrap();

        let result = tool_get_ir_slice(
            tmp.path(),
            json!({"path": "CONVENTIONS.md", "query": "auth"}),
        )
        .unwrap();
        // slice is a string (compact format by default)
        assert!(result["slice"].is_string());
        assert!(result["matched"].is_number());
    }

    #[test]
    fn test_get_ir_slice_query_no_match_returns_empty() {
        let tmp = TempDir::new().unwrap();
        let md = "# Auth\n\nUsers **must** authenticate before accessing any endpoint.\n";
        let md_path = tmp.path().join("CONVENTIONS.md");
        std::fs::write(&md_path, md).unwrap();
        ...
        let result = tool_get_ir_slice(
            tmp.path(),
            json!({"path": "CONVENTIONS.md", "query": "zzz_nonexistent_entity"}),
        )
        .unwrap();
        assert_eq!(result["slice"].as_str().unwrap(), "");
        assert_eq!(result["matched"].as_u64().unwrap(), 0);
    }

    #[test]
    fn test_get_ir_slice_json_format() {
        let tmp = TempDir::new().unwrap();
        let md = "# Auth\n\nUsers **must** authenticate before accessing any endpoint.\n";
        let md_path = tmp.path().join("CONVENTIONS.md");
        std::fs::write(&md_path, md).unwrap();
        ...
        let result = tool_get_ir_slice(
            tmp.path(),
            json!({"path": "CONVENTIONS.md", "format": "json"}),
        )
        .unwrap();
        // In json format, slice is an object, not a string
        assert!(result["slice"].is_object());
        assert!(result["matched"].is_number());
    }

    #[test]
    fn test_get_ir_slice_rejects_absolute_path() {
        let tmp = TempDir::new().unwrap();
        let result = tool_get_ir_slice(tmp.path(), json!({"path": "/etc/passwd"}));
        assert!(result.is_err());
        assert!(format!("{}", result.unwrap_err()).contains("Absolute paths are not allowed"));
    }

    #[test]
    fn test_get_ir_slice_rejects_parent_traversal() {
        let tmp = TempDir::new().unwrap();
        let result = tool_get_ir_slice(tmp.path(), json!({"path": "../../../etc/passwd"}));
        assert!(result.is_err());
        assert!(format!("{}", result.unwrap_err()).contains("Path traversal is not allowed"));
    }
}
